...
class ISAR(BaseAgent):
    def __init__(self, args, env, env_params):
        ...
        self.temperature = self.args.wtd_temperature
    ...
    def _update_network(self, future_p=None):
        ...
        current_v = self.critic_v(inputs_norm_tensor)
        ...
        with torch.no_grad():
            ...
            next_v = self.critic_v(inputs_next_norm_tensor)
            target_q_value = r_tensor + self.args.gamma * next_v.detach()
            # target_q_value[torch.where(r_tensor==0)[0]] = 0
            clip_return = 1 / (1 - self.args.gamma)
            target_q_value = torch.clamp(target_q_value, -clip_return, 0)
        ...
        if self.total_iter % self.policy_delay == 0:
            ...
            with torch.no_grad():
                v = self.critic_v(inputs_norm_tensor)
                ...
                # q = r_tensor + self.args.gamma * torch.min(self.critic_target_network(inputs_next_norm_tensor, actions_next),
                #                                            self.critic_target_network2(inputs_next_norm_tensor, actions_next))
                # # adv = q - v
                q_min = torch.min(self.critic_target_network(inputs_norm_tensor, actions_tensor),
                              self.critic_target_network2(inputs_norm_tensor, actions_tensor))
                adv = q_min - v
                weights = torch.clip(torch.exp(adv) * self.temperature, 0.0, 100.0)
