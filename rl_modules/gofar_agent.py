...
class GoFAR(BaseAgent):
    def __init__(self, args, env, env_params):
        ...
        self.q_critic = critic(env_params)
        # self.q_critic_target = critic(env_params)
        # sync the networks across the cpus
        sync_networks(self.actor_network)
        sync_networks(self.value_network)
        sync_networks(self.q_critic)
        ...
        self.q_critic_optim = torch.optim.Adam(self.q_critic.parameters(), lr=self.args.lr_critic)
    ...
    def _soft_update(self):
        ...
        self._soft_update_target_network(self.q_critic_target, self.q_critic)
    ...
    def _update_network(self, future_p=None):
        ...
        with torch.no_grad():
            action_next = self.actor_network(inputs_next_norm_tensor)
            q_next = self.q_critic_target(inputs_next_norm_tensor, action_next)
            target_q = r_tensor + self.args.gamma * q_next.detach()
            clip_return = 1 / (1 - self.args.gamma)
            target_q_value = torch.clamp(target_q, -clip_return, 0)

        q_current = self.q_critic(inputs_norm_tensor, actions_tensor)
        q_loss = (target_q_value - q_current).pow(2).mean()
        self.q_critic_optim.zero_grad()
        q_loss.backward()
        if self.max_grad_norm is not None:
            nn.utils.clip_grad_norm_(self.q_critic.parameters(), self.max_grad_norm)
        sync_grads(self.q_critic)
        ...
        if self.args.f == 'chi':
            ...
            with torch.no_grad():
                ...
                Q = self.q_critic(inputs_norm_tensor, actions_real)
                v = self.value_target_network(inputs_norm_tensor)
                w_e = torch.clamp((Q-v).exp(), 0, 10)
